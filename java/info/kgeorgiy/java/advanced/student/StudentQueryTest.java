...
public class StudentQueryTest extends BaseTest {
    ...
    private void testList(final Function<List<Student>, List<Student>> query, final int[]... answers) {
        test(query, this::getStudents, answers);
    }

    protected List<Student> getStudents(final List<Student> students, final int[] answer) {
        return Arrays.stream(answer).mapToObj(students::get).collect(Collectors.toList());
    }

    @SafeVarargs
    protected final <T, A> void test(final Function<List<Student>, T> query, final BiFunction<List<Student>, A, T> answer, final A... answers) {
